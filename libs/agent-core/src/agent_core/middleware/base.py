...
class MiddlewareManager:
    ...
    def wrap_node(self, node_func: Callable) -> Callable:
        ...
        async def wrapped_node(state: BaseAgentState) -> BaseAgentState:
...
class ConditionalMiddleware(BaseMiddleware):
    ...
    def __init__(self, condition: Callable[[str, BaseAgentState], bool], name: str = None):
        super().__init__(name)
    ...
    async def _conditional_before_execution(self, node_name: str, state: BaseAgentState) -> BaseAgentState:
        """条件满足时的前置处理"""
        return state
    
    async def _conditional_after_execution(self, node_name: str, state: BaseAgentState, result: Any) -> BaseAgentState:
        """条件满足时的后置处理"""
        return state
    
    async def _conditional_on_error(self, node_name: str, state: BaseAgentState, error: Exception) -> BaseAgentState:
        """条件满足时的错误处理"""
        return state
