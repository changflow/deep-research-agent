...
class LoggingMiddleware(BaseMiddleware):
    ...
    async def before_node_execution(self, node_name: str, state: BaseAgentState) -> BaseAgentState:
        ...
        # 尝试获取研究计划信息（如果是 ResearchAgentState）
        if hasattr(state, "research_plan") and getattr(state, "research_plan"):
            research_plan = getattr(state, "research_plan")
            if hasattr(state, "get_current_step"):
                current_step = state.get_current_step()
                if current_step:
                    current_step_index = getattr(state, "current_step_index", 0)
                    steps_count = len(research_plan.steps) if hasattr(research_plan, "steps") else 0
                    self._logger.info(f"📋 Current step: {current_step.title} ({current_step_index + 1}/{steps_count})")
...
class TracingMiddleware(BaseMiddleware):
    ...
    def __init__(self, settings=None):
        ...
        try:
            ...
            # 打印配置信息以便调试
            if self.settings:
                pk = self.settings.LANGFUSE_PUBLIC_KEY[:5] if self.settings.LANGFUSE_PUBLIC_KEY else "None"
                sk = self.settings.LANGFUSE_SECRET_KEY[:5] if self.settings.LANGFUSE_SECRET_KEY else "None"
                print(f"DEBUG: TracingMiddleware init. PK={pk}... SK={sk}... Host={self.settings.LANGFUSE_HOST}")
            
            # 显式传入配置，确保即使环境变量未设置也能工作
            if self.settings and self.settings.LANGFUSE_PUBLIC_KEY and self.settings.LANGFUSE_SECRET_KEY:
...
class ErrorHandlerMiddleware(BaseMiddleware):
    ...
    async def on_error(self, node_name: str, state: BaseAgentState, error: Exception) -> BaseAgentState:
        ...
        retry_count = _get_attr(state, "retry_count", 0)
        _set_attr(state, "retry_count", retry_count + 1)
        
        current_retry_count = _get_attr(state, "retry_count")
        
        if error_category == "retryable" and current_retry_count <= self.max_retries:
            self._logger.info(f"Error is retryable, attempt {current_retry_count}/{self.max_retries}")
            # 不设置错误状态，让系统重试
            return state
            ...
            if hasattr(state, "set_error"):
                state.set_error(f"Node {node_name} failed: {error}")
            else:
                _set_attr(state, "error_message", f"Node {node_name} failed: {error}")
                _set_attr(state, "status", "error")
    ...
    async def _attempt_recovery(self, node_name: str, state: BaseAgentState, error: Exception) -> BaseAgentState:
        """尝试从错误中恢复"""
        ...
        # 默认恢复：记录错误但继续执行
        metadata = _get_attr(state, "metadata")
        if metadata is None:
            metadata = {}
            _set_attr(state, "metadata", metadata)
        
        if "recovery_attempts" not in metadata:
            metadata["recovery_attempts"] = []
            
        metadata["recovery_attempts"].append({
            "node": node_name,
            "error": str(error),
            "timestamp": datetime.now().isoformat()
        })
...
class PerformanceMiddleware(BaseMiddleware):
    ...
    async def before_node_execution(self, node_name: str, state: BaseAgentState) -> BaseAgentState:
        ...
        metadata[f"{node_name}_perf_start"] = time.perf_counter()
        metadata[f"{node_name}_memory_before"] = self._get_memory_usage()
    ...
    async def after_node_execution(self, node_name: str, state: BaseAgentState, result: Any) -> BaseAgentState:
        ...
        start_time = metadata.get(f"{node_name}_perf_start")
        if start_time:
            duration = time.perf_counter() - start_time
            memory_after = self._get_memory_usage()
            memory_before = metadata.get(f"{node_name}_memory_before", 0)
            ...
            metadata["performance_metrics"].append(metrics)
...
class ContextMiddleware(BaseMiddleware):
    ...
    async def before_node_execution(self, node_name: str, state: BaseAgentState) -> BaseAgentState:
        ...
        context = {
            ...
            "node_name": node_name,
            "session_id": _get_attr(state, "session_id"),
            "timestamp": datetime.now().isoformat(),
            **self.context_data
        }
        ...
        metadata[f"{node_name}_context"] = context
        _set_attr(state, "metadata", metadata)
        return state
